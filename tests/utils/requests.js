...
async function validateValuesInArrayOfObjects(body, fields, context, method, path, headers, response,key, value, requestBody) {
    ...
    if(!objectToValidate){
        assert.fail(key, value, `object with key ${key} and value ${value} is not found`)
    }
    // Alternative:
    // await validateExpectedValues(objectToValidate, fields, context, method, path, headers, response, body)
...
async function validateExpectedTypes(body, fields, context, method, path, headers, response, requestBody) {
    fields.forEach(field => {
        try {
            ...
            //expect(getNestedValue(field.path, body), `${field.path} expected to be type ${field.value}`).to.be.an(field.type)
        } catch (error) {
            addRequestInfoToReport(context, method, path, headers, response, requestBody)
            const actual = typeof getNestedValue(field.path, body)
            assert.fail(actual, field.type, `${field.path} expected type is ${field.type}, but actual was ${actual}`)
        }
    })
}
...
async function validateSchema(body, schema,context){
    const ajv = new Ajv
...
async function performValidations(responseBody, asserts, context, method, path, headers, response, body) {
    ...
    if (asserts.expectedValuesInArrayOfObjects) {
        await validateValuesInArrayOfObjects(responseBody, asserts.expectedValuesInArrayOfObjects.fields, context, method, path, headers, response, asserts.key, asserts.value, body)
    }
